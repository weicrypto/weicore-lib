...
Weicore provides a very simple API for creating transactions. We expect this API to be accessible for developers without knowing the working internals of Wei in deep detail. What follows is a small introduction to transactions with some basic knowledge required to use this API.
...
Note also that an output can be used only once. That's why there's a concept of "change address" in the Wei ecosystem: if an output of 10 WEI is available for me to spend, but I only need to transmit 1 WEI, I'll create a transaction with two outputs, one with 1 WEI that I want to spend, and the other with 9 WEI to a change address, so I can spend this 9 WEI with another private key that I own.
...
Now, this could just be serialized to hexadecimal ASCII values (`transaction.serialize()`) and sent over to the weid reference client.

```bash
wei-cli sendrawtransaction <serialized transaction>
```
...
var transaction = new Transaction().fee(1e8); // Generous fee of 1 WEI
```
...
Transaction inputs are instances of either [Input](../../lib/transaction/input) or its subclasses. `Input` has some abstract methods, as there is no actual concept of a "signed input" in the Wei scripting system (just valid signatures for <tt>OP_CHECKSIG</tt> and similar opcodes). They are stored in the `input` property of `Transaction` instances.

Weicore contains two implementations of `Input`, one for spending _Pay to Public Key Hash_ outputs (called `PublicKeyHashInput`) and another to spend _Pay to Script Hash_ outputs for which the redeem script is a Multisig script (called `MultisigScriptHashInput`).
...
- `script`: the `Script` instance for this input. Usually called `scriptSig` in the Wei community.
...
- `toBuffer`: Serializes the transaction for sending over the wire in the Wei network
- `toBufferWriter`: Uses an already existing BufferWriter to copy over the serialized transaction

## Serialization Checks

When serializing, the Weicore library performs a series of checks. These can be disabled by providing an object to the `serialize` method with the checks that you'll like to skip.
...
These are the current default values in the Weicore library involved on these checks:
...
All Wei transactions contain a locktime field. The locktime indicates the earliest time a transaction can be added to the blockchain. Locktime allows signers to create time-locked transactions which will only become valid in the future, giving the signers a chance to change their minds. Locktime can be set in the form of a Wei block height (the transaction can only be included in a block with a higher height than specified) or a Linux timestamp (transaction can only be confirmed after that time). For more information see [Bitcoin's development guide section on locktime](https://bitcoin.org/en/developer-guide#locktime-and-sequence-number).

In Weicore, you can set a `Transaction`'s locktime by using the methods `Transaction#lockUntilDate` and `Transaction#lockUntilBlockHeight`. You can also get a friendly version of the locktime field via `Transaction#getLockTime`;
...
We're debating an API for Merge Avoidance, CoinJoin, Smart contracts, CoinSwap, and Stealth Addresses. We're expecting to have all of them by some time in 2015. Payment channel creation is available in the [weicore-channel](https://github.com/weicrypto/weicore-channel) module.
