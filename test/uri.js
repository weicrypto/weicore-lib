...
describe('URI', function () {
  ...
  it('parses uri strings correctly (test vector)', function () {
    ...
    uri = URI.parse('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=123.22');
    uri.address.should.equal('Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    uri.amount.should.equal('123.22');
    expect(uri.otherParam).to.be.undefined;

    uri = URI.parse(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=123.22' +
  ...
  it('URIs can be validated statically (test vector)', function () {
    URI.isValid('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG').should.equal(true);
    URI.isValid('wei:yRM2dX5HJyvbFaGubkVPio9W6Y8ELDnJrm').should.equal(true);

    URI.isValid(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2'
    ).should.equal(true);
    URI.isValid(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&other=param'
    ).should.equal(true);
    URI.isValid(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&req-other=param',
      ['req-other']
    ).should.equal(true);
    URI.isValid(
      'wei:ySfKnKxK4S6TkVg6kyUEdXsoLPoCe4YdVp?amount=0.1&' +
        'r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu'
    ).should.equal(true);

    URI.isValid('wei:').should.equal(false);
    URI.isValid('wei:badUri').should.equal(false);
    URI.isValid(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPH?amount=bad'
    ).should.equal(false);
    URI.isValid(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPH?amount=1.2&req-other=param'
    ).should.equal(false);
    URI.isValid(
      'wei:?r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu'
    ).should.equal(false);
  });
  ...
  it('do not need new keyword', function () {
    var uri = URI('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
  ...
  describe('instantiation from bitcoin uri', function () {
    ...
    it('parses address', function () {
      uri = new URI('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    ...
    it('parses a testnet address', function () {
      uri = new URI('wei:yRM2dX5HJyvbFaGubkVPio9W6Y8ELDnJrm');
      uri.address.should.be.instanceof(bitcore.Address);
    ...
    it('throws error when a required feature is not supported', function () {
      (function () {
        return new URI(
          'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&other=param&req-required=param'
        );
      }.should.throw(Error));
    });
  ...
  it('should support double slash scheme', function () {
    var uri = new URI('wei://Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    uri.address.toString().should.equal('Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
  });

  it('should input/output String', function () {
    var str =
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?' +
  ...
  it('should support numeric amounts', function () {
    var uri = new URI(
      'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=12.10001'
    );
  ...
  it('should generate a valid URI', function () {
    new URI({
      ...
      .toString()
      .should.equal('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    ...
    new URI({
      ...
      .toString()
      .should.equal(
        'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.10001&message=Hello%20World&something=else'
      );
  });

  it('should be case insensitive to protocol', function () {
    var uri1 = new URI('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    var uri2 = new URI('wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
  ...
  it('displays nicely on the console (#inspect)', function () {
    var uri = 'wei:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG';
